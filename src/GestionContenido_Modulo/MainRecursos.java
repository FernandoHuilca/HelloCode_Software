...
import GestionContenido_Modulo.Articulo;
import GestionContenido_Modulo.DocumentoPDF;
import GestionContenido_Modulo.RecursoAprendizaje;
import GestionContenido_Modulo.Video;
/**
...
public class MainRecursos {
    ...
    public static void main(String[] args) {
        System.out.println("===== PRUEBA DEL MÓDULO DE RECURSOS DE APRENDIZAJE =====");
        System.out.println("Creando diferentes tipos de recursos...\n");

        // 1. Crear instancias de cada tipo de recurso concreto.
        //    Esto comprueba que los constructores funcionan correctamente.
        Video recursoVideo = new Video(
                "Introducción a las Variables en Java",
                "https://www.youtube.com/watch?v=abcdef123",
                480 // 8 minutos
        );

        Articulo recursoArticulo = new Articulo(
                "Buenas Prácticas de Nomenclatura",
                "https://medium.com/articulos-de-programacion/nomenclatura"
        );

        DocumentoPDF recursoPdf = new DocumentoPDF(
                "Hoja de Referencia Rápida de Sintaxis Java",
                "https://misitio.com/cheatsheet_java.pdf",
                2
        );

        System.out.println("Recursos creados exitosamente:");
        System.out.println("- " + recursoVideo);
        System.out.println("- " + recursoArticulo);
        System.out.println("- " + recursoPdf);

        System.out.println("\n===== PROBANDO POLIMORFISMO Y MÉTODO ABSTRACTO =====");

        // 2. Usar polimorfismo: guardar todos los recursos en una lista del tipo base.
        //    Esto es lo que harías en la clase NodoRuta al agregar material de apoyo.
        List<RecursoAprendizaje> materialDeApoyo = new ArrayList<>();
        materialDeApoyo.add(recursoVideo);
        materialDeApoyo.add(recursoArticulo);
        materialDeApoyo.add(recursoPdf);

        System.out.println("Iterando sobre una lista de RecursoAprendizaje y llamando al método obtenerDetalle():\n");

        // 3. Iterar sobre la lista y llamar al método que tiene un comportamiento diferente
        //    en cada subclase. Esto demuestra que el método abstracto `obtenerDetalle()`
        //    está implementado correctamente en cada clase hija.
        for (RecursoAprendizaje recurso : materialDeApoyo) {
            // La variable 'recurso' es de tipo RecursoAprendizaje,
            // pero Java sabe qué implementación específica de obtenerDetalle() llamar en tiempo de ejecución.
            System.out.println("----------------------------------------");
            System.out.println("Título: " + recurso.getTitulo());
            System.out.println("Tipo: " + recurso.getClass().getSimpleName());
            System.out.println("Acción Sugerida: " + recurso.obtenerDetalle()); // <-- Aquí ocurre la magia del polimorfismo
        }
        System.out.println("----------------------------------------");

        System.out.println("\nPrueba finalizada. La jerarquía de recursos funciona como se esperaba.");
    }
}
 **/
import java.util.ArrayList;
